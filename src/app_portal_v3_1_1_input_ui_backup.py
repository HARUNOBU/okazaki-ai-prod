...
# .env を「プロジェクトルート」から読む（実行場所がどこでもOK）
PROJECT_ROOT = Path(__file__).resolve().parents[2]  # .../okazaki-ai-portal
load_dotenv(PROJECT_ROOT / ".env")

DEFAULT_DB_DIR = PROJECT_ROOT / "chroma_db"

# 必要なら .env で上書き可能（通常は触らない）
WASTE_LIFE_DB_DIR = Path(os.getenv("WASTE_LIFE_DB_DIR", str(DEFAULT_DB_DIR)))
KANKO_DB_DIR      = Path(os.getenv("KANKO_DB_DIR",      str(DEFAULT_DB_DIR)))
...
# sidebar の slider を差し替え
    ...
    q = st.text_input("質問を入力してください", key="q", placeholder="例：岡崎城と周辺を半日で回るなら？")
    if not q:
        st.stop()

    # カテゴリ指定モードの場合は、ルータを使わず指定カテゴリを優先
    if mode == "カテゴリ指定":
        fixed_cat = st.selectbox("カテゴリを選択", list(COLLECTIONS.keys()))
        # fixed_cat を強制するなら answer_portal に渡す設計にするのが筋だが、
        # いまは最小修正として「質問文にカテゴリを補助情報として付ける」方法で対応（誤分類を減らす）
    ...
    forced_cat = None
    if mode == "カテゴリ指定":
        forced_cat = st.selectbox("カテゴリを選択", list(COLLECTIONS.keys()))
    ...
    st.write(
        f"判定カテゴリ: **{chosen_cat}** / collection: `{chosen_col}` / best_dist: {best_dist:.3f} / threshold: {DIST_THRESHOLD}"
    )
