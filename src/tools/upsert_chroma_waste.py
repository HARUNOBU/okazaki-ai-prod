...
COLLECTION_NAME = "okazaki_waste"
DOCS_JSON = DATA_DIR / "docs" / "docs_okazaki_waste.json"
...
def main():
    ...
    for id_batch, text_batch, meta_batch in zip(
        ...
        emb = oa.embeddings.create(model=EMBED_MODEL, input=text_batch)
        vectors = [e.embedding for e in emb.data]

        if hasattr(col, "upsert"):
            col.upsert(ids=id_batch, embeddings=vectors, documents=text_batch, metadatas=meta_batch)
        else:
            col.add(ids=id_batch, embeddings=vectors, documents=text_batch, metadatas=meta_batch)

        print(f"added: {len(id_batch)}")
