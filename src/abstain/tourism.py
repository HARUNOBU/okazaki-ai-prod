...
# 1.5) 固定情報（dist で落としてはいけない）
# 例：営業時間/料金/住所/URL/問い合わせ/アクセス/駐車場/最寄り駅など
FIXED_INFO_PATTERNS: list[tuple[str, str]] = [
    (r"(営業時間|開館時間|開館|休館|定休日|時間は|何時から|何時まで)", "hours"),
    (r"(料金|入場料|入館料|拝観料|金額|いくら|無料|有料|チケット)", "price"),
    (r"(住所|所在地|場所|どこ|地図|マップ)", "address"),
    (r"(公式サイト|ホームページ|URL|リンク)", "url"),
    (r"(電話|連絡先|問い合わせ|問合せ|お問い合わせ)", "contact"),
    (r"(アクセス|行き方|最寄り駅|駅から|バスで|車で|駐車場)", "access"),
]
...
def decide_tourism_abstain(query: str, best_dist: float, dist_threshold: float) -> TourismAbstainDecision:
    ...
    if not q:
        ...
        # 1) HARD（リアルタイム要求は即 abstain）
    hit, tag, frag = _match_any(HARD_PATTERNS, q)
    if hit:
         # app側が見ている reason="keyword" に統一
        return TourismAbstainDecision(True, "keyword", f"{tag}:{frag}")
 
    # 1.5) 固定情報 intent（営業時間/料金/住所/URL/問い合わせ/アクセス等）
    # → dist が悪くても abstain しない（「RAG不足」を判定ロジックで誤爆させない）
    fixed_hit, fixed_tag, fixed_frag = _match_any(FIXED_INFO_PATTERNS, q)
    if fixed_hit:
        return TourismAbstainDecision(False, "ok", f"fixed:{fixed_tag}:{fixed_frag}")
    ...
    if soft_hit:
        ...
        # SOFTは「例年・一般論」扱い。ここでは abstain しない。
        # 根拠不足の扱いは app側の main_pass / cite有無 / メッセージ分岐で処理する。
