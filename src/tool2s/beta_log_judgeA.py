...
def judge_one_case(
    ...
    result["matched_run_id"] = str(matched_log.get("run_id", ""))
...
def main() -> int:
    ...
    parser.add_argument("--run-id", default="", help="judge only logs with this run_id")
    ...
    if run_id:
        logs = [x for x in logs if norm_text(x.get("run_id", "")) == run_id]
